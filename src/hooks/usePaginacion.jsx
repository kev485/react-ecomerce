...
export const usePaginacion = (nombreColeccion, campoOrden = "nombre", itemsPorPagina = 8) => {
    ...
    // Función común para obtener el total de paginas
    ...
    const cargarPagina = async (numeroPagina) => {
        ...
        try {
            let consulta;
            //Crear la consulta segun la página
            if (numeroPagina === 1) {
                ...
                consulta = query(
                    collection(db, nombreColeccion),
                    orderBy(campoOrden),
                    startAfter(documentoAnterior), //Le decimos que empiece a leer despues de ese marcador
                    limit(itemsPorPagina)
                );
            }
            ...
            //Guardar los datos en el estgado para que se muestren en pantalla
            setData(items);
            ...
            if (!historialDocs[numeroPagina] && snapshot.docs.length > 0) {
                //Agregamos el último documento de la lista que nos devolvió Firebase
    ...
    //Función auxiliar para refrescar la página actual tras un CRUD
